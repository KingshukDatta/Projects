...
while True:
  ...
  # Deleting a student's record from the database
    ...
    name = input("Enter the desired student's name to delete: ")
    # SQL query to delete a student record based on the name provided by the user.
    sql = "DELETE FROM students WHERE name = %s"
    cursor.execute(sql, (name,))
    # checking if any record was deleted and printing the appropriate message.
    if cursor.rowcount > 0:
      print(f"Student '{name}' deleted successfully.")
      db.commit()
    else:
      print(f"No student found with the name '{name}'.")
      db.rollback()  # Rollback the transaction if no record was deleted.
